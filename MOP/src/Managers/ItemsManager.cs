...
namespace MOP.Managers
{
    class ItemsManager : IManager<ItemBehaviour>
    {
        ...
        private void CreateRadiatorHoseFix()
        {
            ...
            GameObject[] attachedHoses = Resources.FindObjectsOfTypeAll<GameObject>().Where(g => g.name == "radiator hose3(xxxxx)").ToArray();
            if (attachedHoses.Length == 0)
            {
                ModConsole.LogError("[MOP] Could not find radiator hose3(xxxxx) object");
                return;
            }
            GameObject attachedHose = attachedHoses[0];
            
            GameObject[] realHoses = Resources.FindObjectsOfTypeAll<GameObject>().Where(g => g.name == "radiator hose3(Clone)").ToArray();
            if (realHoses.Length == 0)
            {
                ModConsole.LogError("[MOP] Could not find radiator hose3(Clone) object");
                return;
            }
            realRadiatorHose = realHoses[0];
        ...
        private static void LoadStoreHook()
        {
            ...
            postOrder.FsmInject("State 3", GameObject.Find("STORE/StoreCashRegister/Register").AddComponent<CashRegisterBehaviour>().Packages);
        ...
        private void InitializeList()
        {
            ...
            // CD Player Enhanced compatibility - temporarily disabled
            // if (MSCLoader.ModLoader.Mods.Any(m => m.ID == "CDPlayer"))
            if (false)
            {
