...
namespace MOP.DebugTools
{
    class DebugMonitor : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (guiObject == null)
            {
                ModConsole.LogError("[MOP] Could not find GUI object for debug monitor");
                return;
            }

            Transform fpsTransform = guiObject.transform.Find("HUD/FPS/HUDValue");
            if (fpsTransform == null)
            {
                ModConsole.LogError("[MOP] Could not find HUD/FPS/HUDValue object for debug monitor");
                return;
            }

            GameObject fpsObject = GameObject.Instantiate(fpsTransform.gameObject);
