...
namespace MOP.Vehicles.Managers
{
    class DelayedHingeManager : MonoBehaviour
    {
        ...
        IEnumerator InitializationRoutine(bool noDelay = false)
        {
            ...
            if (!initialHookDone)
            {
                // FsmInject temporarily disabled due to API conflicts
                // MOP.FSM.FsmExtensions.FsmInject(GameObject.Find("SATSUMA(557kg, 248)").transform.Find("Body/trigger_hood").gameObject, "Assemble 2", UpdateInitialRotation);
